...
public class OOPBasic {
    public static void main(String[] args) {
        Student s1 = new Student("안유진", "서울시 강남구 역삼동"); // Student 클래스로 s1 객체 생성
        Student s2 = new Student("장원영", "서울시 강남구 청담동"); // Student 클래스로 s2 객체 생성
        System.out.println(s1.setStudentName(null)); // 이름이 있으면 200 반환
        s1.showStudentInfo();
        s2.showStudentInfo();
    }
}
...
class Student {
    ...
    // 반환 타입 : 메소드가 작업을 마치고 반환 하는 데이터 타입을 명시 (void는 반환 할 게 없다는 의미)
